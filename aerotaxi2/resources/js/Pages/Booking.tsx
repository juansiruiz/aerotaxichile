...
function BookingWizard() {
  ...
  // Use URLSearchParams for search params in Inertia
  ...
  const [direction, setDirection]     = useState<'to_airport' | 'from_airport'>(
    (queryParams.get('direction') as 'to_airport' | 'from_airport') ?? 'to_airport',
  )
  const [date, setDate]               = useState(queryParams.get('date') ?? '')
  const [time, setTime]               = useState(queryParams.get('time') ?? '')
  const [zoneId, setZoneId]           = useState(queryParams.get('zoneId') ?? '')
  const [vehicleType, setVehicleType] = useState(queryParams.get('vehicleType') ?? 'sedan')
  ...
  const [fromCommune,    setFromCommune]    = useState(queryParams.get('fromCommune') ?? '')
  const [toCommune,      setToCommune]      = useState(queryParams.get('toCommune') ?? '')
  const [communeRouteId, setCommuneRouteId] = useState(queryParams.get('communeRouteId') ?? '')
  ...
  // ─────────────────────────────────────────────────────────────────────────
  if (confirmed) return <ConfirmationScreen confirmed={confirmed} onDashboard={() => router.visit('/dashboard')} />
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-100 sticky top-0 z-20 shadow-sm">
        <div className="max-w-xl mx-auto px-4 py-3 flex items-center gap-3">
          <button
            onClick={() => (step > 1 ? setStep((step - 1) as 1 | 2 | 3) : window.history.back())}
...
function ConfirmationScreen({
  ...
  const wa = import.meta.env.VITE_WHATSAPP_NUMBER ?? '56963552132'
